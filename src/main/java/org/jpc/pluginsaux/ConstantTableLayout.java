...
public class ConstantTableLayout implements LayoutManager2
{
    ...
    public void layoutContainer(Container co)
    {
        ...
        double yscale = co.getHeight() / (double)(rowHeight * rows) -0.2; //TODO: set MARGIN

        for(Map.Entry<Component, Placement> entry : components.entrySet()) {
            Placement p = entry.getValue();
            entry.getKey().setBounds((int)(p.posX * columnWidth * xscale),
                    (int)(p.posY * rowHeight * yscale) +50, //TODO: set MARGIN
                (int)(p.sizeW * columnWidth * xscale),
                (int)(p.sizeH * rowHeight * yscale));
        }
    }
